...
class WandBProvider(OpenAICompatProvider):
    """Weights & Biases Serverless Inference — OpenAI-compatible endpoint.
    Hosts open-source models (Llama, DeepSeek, Qwen, Kimi, Phi, etc.).
    Auth: Bearer <WANDB_API_KEY>. Base URL: https://api.inference.wandb.ai/v1
    Requires OpenAI-Project header — set WANDB_PROJECT=<entity>/<project>.
    Rate model: concurrency-based (2000 concurrent/user); no hard RPM/RPD.
    """
    name = "wandb"
    capabilities = {**OpenAICompatProvider.capabilities, "reasoning": True}
    ...
    def __init__(self, api_key, model):
        ...
        # Required by WandB Inference: identifies the W&B entity/project to bill.
...
def model_capabilities(provider_name: str, model: str, default_caps: dict) -> dict:
    ...
    if provider_name in ("groq", "cerebras", "nvidia", "openrouter", "github", "wandb"):
...
def build_providers(cache_store):
    ...
    if k := os.getenv("WANDB_API_KEY"):
        out["wandb"] = WandBProvider(k, os.getenv("WANDB_MODEL", "Qwen/Qwen3-27B"))
...
def build_router_providers():
    ...
    if k := os.getenv("WANDB_API_KEY"):
        out["wandb"] = WandBProvider(k, os.getenv("ROUTER_WANDB_MODEL", ROUTER_DEFAULTS["wandb"]))
    return out
