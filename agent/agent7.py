...
async def _interactive_loop() -> None:
    ...
    print("Agent7 — type a query and press Enter.  Type 'quit' or Ctrl-C to exit.\n")
    while True:
        try:
            query = input("agent7> ").strip()
...
def main() -> None:
    ...
    if "-i" in args or "--interactive" in args:
        ...
        if not query:
            query = "What is the current time in Asia/Tokyo and Asia/Kolkata? Tell me the difference in hours."
